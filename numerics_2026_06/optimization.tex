...
$$
Equation~\eqref{eq:cf4} instead combines two exponentials with the two generators
\begin{equation*}
G_A=a_2H_1+a_1H_2,
\qquad
G_B=a_1H_1+a_2H_2.
...
We can also use this method to evolve the experimental curve:
\begin{align}
...
\label{eq:f20}
\end{equation}
We minimize it with multistart Nelder--Mead searches.  
...
After the optimization, we use the CF4-Krylov time evolution of the full Hamiltonian to compute the true fidelity; the results are summarized in Figure~\ref{fig:fidelity}. The coefficients of the optimized curve are summarized in Table~\ref{tab:path-coefficients}.
