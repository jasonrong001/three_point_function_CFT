...
\caption{The phase diagram of the Rydberg model \eqref{eq:model}. The red circle corresponds to the point studied in~\cite{Fang:2024eaw}. The red circle corresponds to the point with exact $\mathbb{Z}_2$ symmetry.} 
...
The non-universal constant $b$ is model dependent. We take $L=24$, at $\Delta'=\zeta(6)$, the critical point is located at $\Omega'=0.488$. The two point function is shown in Fig.~\ref{ZZ2pt}. We have used the definition
$$
...
\caption{The correlation function $(-1)^x \langle Z_i Z_{x+i}\rangle$. The red, green, blue curves correspond to $\Omega'=0.488,~0.45,$ and $0.52$ respectively. The black dashed line is $\frac{0.645}{|x_L|^{1/4}}$.} 
\label{ZZ2pt}
\end{figure}
...
\eeq
We take take $\Delta'_i=1.01734$ and $\Omega=0.488$, then the bulk is in the critical phase. At the boundary, this breaks the $\mathbb{Z}_2$ symmetry, and prefers $\langle Z_i\rangle<0$. 
We can further tune $\Delta'$ on the boundary to be 
\beq\label{boundarytuning}
\Delta'_1=\Delta'_N=5.
\eeq
I believe a spatial dependent $\Delta'$ is not hard, as discussed in \cite{Sun:2026aqf}, see the section ``Boundary tuning of TCI CFT''. The precise value of the detuning on the boundary is not crucial, as long as it is strong enough to fixed the boundary spin.
Notice that we have to put odd number of atoms, due to the anti-ferromagnetic nature of model, we take $N=25$. The one-point function we got is shown in Fig.~\ref{Z1pt}.
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Z1pt.png}
\caption{The correlation function $|\langle Z_x \rangle|$. The curve corresponds to $\Delta'_1=\Delta'_N=5$. The black dashed line is $\frac{0.88}{|x_L|^{1/8}}$.} 
\label{Z1pt}
\end{figure}

If we instead take $N$ to be even, for example $N=24$, this means that we are taking spin-up boundary condition on one end and the spin-down boundary condition on the other end. Performing a conformal map to upper half plane, this means we have put a pair of boundary changing operators, one at the origin and the other at infinity. The 3rd operator $\sigma$ operator is at the unit semi-circle.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Z1pt_est.png}
\caption{The correlation function $|\langle Z_x \rangle|$ from sampling the ground state.  We have taking the sample size $N=\times10^4$.}
\label{Z1ptest}
\end{figure}

One can also sample the ground state. The result is summarized in Fig.~\ref{Z1ptest}. Notice due to the lack of translational symmetry, we need more sample as compared to the two point function of periodic spin chain.
