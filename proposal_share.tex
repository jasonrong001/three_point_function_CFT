...
\beq
t_{\rm exp} E_{\rm gap}/ \hbar  \gg 2\pi 
\label{eq:cond}
\eeq
...
Let's suppose the atoms are separated by $a=10\mu\rm m$. Important energy quantity $U=C_6/a^6$ is then \cite[Tab. 1]{noise-note}
\beq
U/\hbar = 0.86\ {\rm\ rad }\cdot {\rm \mu s}^{-1}
\eeq 
(Confirmed by Guillaume; seems to disagree with email \cite{emailB} which said "$C_6/a^6 = 2\pi\ {\rm MHz}$").
...
\beq
(E_{\rm gap})_{phys} /\hbar = 0.0028 {\rm\ rad }\cdot {\rm \mu s}^{-1},
\eeq
So we have 
\beq
(E_{\rm gap})_{phys}/\hbar/(2\pi)\  \approx 450\ {\rm Hz}
\eeq
We fail the adiabaticity condition \eqref{eq:cond} by a factor of about 200.
