...
We can perform a similar study for the $\langle\epsilon(0)\epsilon(x)\rangle$ two point function, with $10^5$ collapsed states. The results are shown in Fig~\ref{fig:eps-eps-sample}.

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.5]{fig-eps-eps-sample-10e5.pdf}
	\includegraphics[scale=0.5]{fig-eps-eps-sample-rserror.pdf}
	\caption{The two-point function $\langle \epsilon(0)\epsilon(x)\rangle$ from sampling the ground state, we have collected $10^5$ collapsed states. Left- the estimation of $\langle \epsilon(0)\epsilon(x)\rangle$. Right- the relative statistical error, that is, the statistical error divided by the CFT prediction.}
