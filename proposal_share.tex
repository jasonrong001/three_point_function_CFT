...
Similarly, we can map the CFT operator $\epsilon$ to the lattice operator as 
\beq
\epsilon(i+1/2)=c_2 \big( (n_{i-1} -n_{i})(n_{i+1} -n_{i+2}) - b \big)
\label{eq:epslat}
\eeq
We take 
\beq
b =\langle (n_{i-1} -n_{i})(n_{i+1} -n_{i+2}) \rangle =0.488577,
\eeq
and 
\beq
c_2 = 2.05816,
\eeq
The DMRG result are summarised in Fig.~\ref{fig:eps-eps}.

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.6]{fig-eps-eps.pdf}
	\caption{The two-point function $\langle \epsilon(0)\epsilon(x)\rangle$ for $1\le |x|\le 20$, plotted in log-log-scale as a function of $|x|_L$, see Eq.~\eqref{eq:xL}. Solid blue line - exact CFT prediction $(|x|_L)^{-2}$. Red dots - our determination using DMRG and lattice operator \eqref{eq:epslat}. }
	\label{fig:eps-eps}
\end{figure}

After fixing the map from CFT operator to lattice, we can now measure the three point function using DMRG, as shown in Fig.~\ref{fig:eps-sig-sig}.
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.7]{fig-eps-sig-sig-theory.pdf}
		\includegraphics[scale=0.7]{fig-eps-sig-sig-DMRG.pdf}
	\caption{The three-point function $\langle \epsilon(1)\sigma(x_2)\sigma(x_3)\rangle$. Left - exact CFT predictio. Right - our determination using DMRG. }
