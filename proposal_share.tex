...
We can perform a similar study for the $\langle\epsilon(0)\epsilon(x)\rangle$ two point function. As this function decreases fast with the distance, we expect that a much larger number of collapsed states will be needed. Here we take $N=10^5$. The results are shown in Fig~\ref{fig:eps-eps-sample}. We see that the relative statistical error grows from a few percent at small separation to  about 20\% at the largest separation.
...
For the study of the three point function  $\langle \epsilon(0)\sigma(x_2)\sigma(x_3)\rangle$, when averaging over the all configurations on a single collapsed state, we need to take in account the configurations related by reflection. The estimation and relative statistical error are given in Fig.~\ref{fig:eps-sig-sig-sample}.
\begin{figure}[htbp]
	\centering
