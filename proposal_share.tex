...
\begin{equation}
    \hat{H}=\frac{\Omega}{2} \sum_i (a_i^{\dagger}+a_i) - \Delta \sum_i n_i + C_6\sum_{i<j}\frac{1}{(d_{ij})^6}  n_{i}n_{j}.
    \label{eq:model}
\end{equation}
...
In the experimental setup, the spin chain is initially prepared with Hamiltonian parameters different from the ones corresponding to the critical point, and then the parameters are varied over the duration of the experiment, which is $t_{\rm exp}\sim 10\mu{\rm s}$ \cite[Sec. II]{noise-note}. We would like that this evolution can be considered adiabatic, so that the spin chain Hamiltonian remain in is ground state during this evolution. The condition for this is
\beq
t_{\rm exp} \omega_{\rm gap}\gg 1
\eeq
where $\omega_{\rm gap}=E_{\rm gap}/\hbar$ and $E_{\rm gap}$ is the energy gap. This energy gap will be smallest at the end of the evolution, when we reach the critical point.

Let's suppose the atoms are separated by $a=10\mu\rm m$. Important energy quantity $C_6/a^6$ is then \cite[Tab. 1]{noise-note}
\beq
C_6/a^6 = \hbar\ 0.86\times 10^6{\rm\ rad }\cdot {\rm s}^{-1}
\eeq 
(cf the value quoted in \cite{emailB} $C_6/a^6 = 2\pi\ {\rm MHz}$ which if we understand the notation appears factor $2\pi$ higher).

One important quantity below is the gap of the quantum spin chain. The critical energy gap is $E_{\rm gap} = E_1-E_0=0.00327$ for $L=40$ in dimensionless units \junchen{this number can be $\times 3$ if we change the location of the critical point on critical line}. In our DRMG simulations we set $C_6/a^6=1$. Using the physical value of $C_6/a^6$ we get in physical units 
\beq
\omega_{\rm gap} \approx 3 \text{ kHz}
\eeq
So we fail the adiabaticity condition by many orders of magnitude.
