...
Let us now discuss the three point function $\langle\epsilon(x_1) \sigma(x_2)\sigma(x_3)\rangle $.  We can set $x_1=0$ by translation invariance. The plots for the theoretical prediction and the DMRG are shown in Fig.~\ref{fig:eps-sig-sig}. In the left panel we show the exact CFT prediction, and in the right panel the relative error between DMRG and CFT (i.e.~$|f_{\rm DMRG}/f_{\rm CFT}-1|$). Some comments are in order. Note that the CFT prediction is a function which blows up as $x_2$ or $x_3$ approach 0, and goes to zero as $x_2,x_3$, see Eq.~\eqref{eq:ess}. This function is symmetric under the interchange of $x_2$ as $x_3$ and also under the flip of the circle orientation which maps $x_2\to L-x_2$, $x_3\to L-x_3$.
