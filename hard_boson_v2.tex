...
There exists a model that is closer to the Rydberg atom experiments, called the Fendley-Sengupta-Sachdev model. {\red The Hamiltonian is 
\begin{equation}
...
Consider any lattice operator $O_{lat}(i)$ on a collection of sites $S$ around a point $i$. This lattice operator will be matched to a linear combination of CFT operators at point $i$:
\beq
O_{lat}(i) = \sum c_\alpha O_{CFT}^\alpha(i)
\eeq
Here $O_{CFT}^\alpha(i)$ are CFT operators and coefficients $c_\alpha$ depend on the operator $O_{lat}$. 
Unit operator will also appear in this list, in case we have a nonzero vev.

Now consider the same lattice operator shifted by one lattice site. The claim is that this operator will match on the same linear combination of CFT operators, except up to inverting the signs of $\mathbb{Z}_2$ odd operators:
\beq
O_{lat}(i+1) = \sum_{\alpha\text{ $\mathbb{Z}_2$ even}} c_\alpha O_{CFT}^\alpha(i+1)- \sum_{\alpha\text{ $\mathbb{Z}_2$ odd}} c_\alpha O_{CFT}^\alpha(i+1)
\eeq
I could give some rationale behind this rule, but let me skip it for now.

This implies that $O_{lat}(i)-O_{lat}(i+1)$ will match to $\mathbb{Z}_2$ odd operators, while $O_{lat}(i)+O_{lat}(i+1)$ will match to $\mathbb{Z}_2$ even operators.

Hence let us consider the operators
\beq
n_i-n_{i+1}
\eeq
and
...
n_i+n_{i+1} - VEV
\eeq
By the above discussion these will map to $(-1)^i \sigma(i+1/2) $ and $\epsilon(i+1/2)$ plus subleading corrections.
